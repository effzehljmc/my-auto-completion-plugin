...
import { intoMyAutoCompletionPath } from "../settings";
...
export const SuggestionBlacklist = new class {
    ...
    async saveData(vault: Vault) {
        await vault.adapter.write(intoMyAutoCompletionPath(vault, BLACKLIST_PATH), [...this.blacklist].join("\n"));
    }

    async loadData(vault: Vault) {
        const path = intoMyAutoCompletionPath(vault, BLACKLIST_PATH);
